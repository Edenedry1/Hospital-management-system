...
def patients_for_secretary():
    ...
    if request.method == 'POST':
        ...
        if 'submit' in request.form.keys():
            ...
            user.is_approved = 1
            db.session.commit()
        if 'read_file' in request.form.keys():
            file_path = fr'{os.path.dirname(__file__)}\patient_files\{patient_id}'
            if os.path.isfile(file_path):
                with open(file_path, 'r') as f:
                    return render_template('text_file.html', patient_id=patient_id, text=f.read())
